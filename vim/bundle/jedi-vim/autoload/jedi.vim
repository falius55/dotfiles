...
function! s:init_python()
    ...
    " Handle "auto" version.
    " has()はvimが特定の機能を持っているかどうか
    " nvimはvimからフォークした新しいvim(NeoVim)
...
function! jedi#setup_py_version(py_version)
    " #3
    if a:py_version == 2
        let cmd_init = 'pyfile'  " 接頭詞がなければ、関数内なら関数ローカルになる
        let cmd_exec = 'python'
    ...
    try
        " initialize.pyファイルをpythonで実行
        " 直接実行でなく文字列なのは、スクリプトのパスが文字列であるため連結して実行する必要があるため
        " initialize.pyでは、基本的にscript_pathをシステムのPATHに含めているだけ
        execute cmd_init.' '.s:script_path.'/initialize.py'
    catch
        throw "jedi#setup_py_version: ".v:exception
    endtry
    " PythonJediコマンドを作成。引数がpythonコマンドに渡される
...
function! jedi#add_goto_window(len)
    ...
    " belowrightはコマンドを実行する
    " コマンドが画面分割を含んでいれば、垂直分割ならば右側に、水平分割ならば下側にカレントウィンドウを出現させる
    execute 'belowright copen '.height
    set nolazyredraw
    if g:jedi#use_tabs_not_buffers == 1
        noremap <buffer> <CR> :call jedi#goto_window_on_enter()<CR>
    endif
    " 新しく作成したウィンドウから離れたら自動的に閉じる
...
function! jedi#goto_window_on_enter()
    let l:list = getqflist()  " quickfixの各行を表す辞書のリスト
    let l:data = l:list[line('.') - 1]  " カーソルの一行上の行番号
    if l:data.bufnr  " 該当行の表すQuickFixエラーのバッファ番号
        " close goto_window buffer
        " normalはノーマルモードのコマンドを実行
        normal ZQ
