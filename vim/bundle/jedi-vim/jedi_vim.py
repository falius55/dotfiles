...
# is_eval trueであればeval(a:str)の結果をリターン
# falseで、execute a:strを実行して空文字を返す
def _catch_exception(string, is_eval):
    ...
    return result['result']  # 例外が起これば複数の値が返ってくるので辞書で返してきている
...
def goto(mode="goto", no_output=False):
    ...
    :rtype: list
    """
    script = get_script()  # 現在バッファのテキストをソースにしてjediからscriptを取得
    try:
        if mode == "goto":
